...
#define AE_CONSTEL_N_CON        89  //!< The number of constellations defined.
...
#define AE_CONSTEL_N_GREEK 24       //!< The number of Greek letters.
...
//!
//! \param cat_name The star name from the catalogue.  It should be of the form
//! {2-char Greek letter abbrev.}[-#]{3-char constellation name}({star name}).
//!                 For example: `beCet(Diphda)', `al-2Lib(Zubenelgen)'.
//! \param full_name For returning an expanded star name; set to NULL if you
//!                  do not need this.
//! \param len The length of \p full_name; ignored if \p full_name is NULL.
//!
//! \return The index of #ae_constel_name representing the constellation that
//!         this star lives in; or -1 if it cannot be found.
//------------------------------------------------------------------------------

int ae_cat_to_constel_index(const char *cat_name, char *full_name, int len) {
  int i, ret = -1;
  char *g, *p, *q, *in_cpy, ans[128], tmpstr[128];

  // Make local copies.
  in_cpy = (char *)malloc((strlen(cat_name) + 1) * sizeof(char));
  p = in_cpy;
  strcpy(p, cat_name);
  q = ans;
  ...
  *q++ = ' ';
  ...
  // Copy any initial Greek letter.
  if (islower(*p)) {
    for (i = 0; i < AE_CONSTEL_N_GREEK; i++) {
      g = ae_greek_letter_name[i];
      if ((*p == *g) && (*(p + 1) == *(g + 1)))
        break;
    }
    if (i < AE_CONSTEL_N_GREEK) {
      while (*g != '\0')
        *q++ = *g++;
    }
    while (islower(*p))
  ...
  // Skip any other junk until the constellation name starts.
  ae_skip_white(p);
  while ((*p != '\0') && !isupper(*p))
    *q++ = *p++;
  *q++ = ' ';
  ...
  if (isupper(*p)) {
	...
	for (i = 0; i < AE_CONSTEL_N_CON; i++) {
      strcpy(tmpstr, ae_constel_name[i]);
      g = tmpstr;
      if( (*p == *g) && (*(p + 1) == *(g + 1)) && (*(p + 2) == *(g + 2)))
        break;
    }

    // Get the name, if found.
	if (i < AE_CONSTEL_N_CON) {
      g += 4;
      while( *g != '\0' )
        *q++ = *g++;
      p += 3;
...
//! \param epoch The Julian date for the ra/dec coordinates.
//!
//! \return The index of #ae_constel_name representing the constellation that
//!         this coordinate lives in; or -1 if it cannot be found.
//------------------------------------------------------------------------------
...
int ae_coord_to_constel_index(double ra, double dec, double epoch) {
  ...
  ae_polar_to_rect(ra, dec, 1.0, p);

  // Precess from given epoch to J2000.
  ae_precess(epoch, p, AE_TO_J2000);
  // Precess from J2000 to Besselian epoch 1875.0.
  ae_precess(2405889.25855, p, AE_FROM_J2000);

  d = p[0] * p[0] + p[1] * p[1] + p[2] * p[2];
  d = sqrt(d);
  ra2 = atan2(p[1], p[0]) * (AE_RTD * 3600.0 / 15.0);
  if (ra2 < 0.0)
    ra2 += 86400.0;
  dec2 = asin (p[2] / d) * (AE_RTD * 3600.0);

  // Find a constellation such that the declination entered is higher than the
  // lower boundary of the constellation when the upper and lower right
  // ascensions for the constellation bound the entered right ascension.
  for (i = 0; i < AE_CONSTEL_N_BOUNDARIES; i++) {
    k = i << 2;
    if (ra2 >= ae_constel_boundary[k] && ra2 < ae_constel_boundary[k + 1] && 
        dec2 > ae_constel_boundary[k + 2]) {
      k = ae_constel_boundary[k + 3];
      return k;
    }
  }
